...
// Forward wheel events from the popup to the map canvas to allow zooming/rotating when scrolling over the popup
map.getContainer().addEventListener('wheel', (e) => {
  const popup = e.target.closest('.maplibregl-popup');
  if (popup) {
    e.preventDefault();
    const canvas = map.getCanvas();
    const forwardEvent = new WheelEvent('wheel', {
      deltaX: e.deltaX,
      deltaY: e.deltaY,
      deltaZ: e.deltaZ,
    ...
    canvas.dispatchEvent(forwardEvent);
  }
}, { passive: false });
...
function applyTerrainState() {
  const terrainMode = document.getElementById('terrain-select').value;
  const exInput = document.getElementById('terrain-exaggeration');
  const viz = (terrainMode === 'hillshade' || terrainMode === '3d') ? 'visible' : 'none';
...
// Removed to restore native pinch-zoom smoothness and inertia.
...
map.on('load', () => {
  ...
  const initialTileUrl = baseUrl + `${tilePrefix}/tiles_koppen/{z}/{x}/{y}.png?v=3`;
  ...
  const storedTerrain = localStorage.getItem('climate_terrain');
  if (storedTerrain) {
    terrainSelect.value = storedTerrain;
  } else {
...
document.getElementById('terrain-select').addEventListener('change', (e) => {
  localStorage.setItem('climate_terrain', e.target.value);
  applyTerrainState();
});
...
terrainExInput.addEventListener('change', () => {
  ...
  if (document.getElementById('terrain-select').value === '3d') {
    if (val > 0) {
      ...
      map.setTerrain(null);
      // Directly reset MapLibre internal state that setTerrain(null) doesn't clear,
      // to match the exact state on a fresh page load where setTerrain was never called.
      try {
        if (map.painter && map.painter.terrain) map.painter.terrain = null;
        if (map.transform && map.transform._elevation !== undefined) map.transform._elevation = 0;
        if (map.transform && map.transform.elevation !== undefined) map.transform.elevation = 0;
...
function updateLayers() {
  const era = eraSelect.value;
  const comp = compareSelect.value;
  let isCompare = (comp !== 'none' && era !== comp);

  // FORCE ANNUAL FOR COMPARE
  // Comparison tiles only exist for annual averages (month 12)
  // and only for Temp, Precip, and Koppen layers.
  if (isCompare) {
    currentMonth = "12";
    if (currentLayerType === 'precip_max' || currentLayerType === 'precip_min') {
  ...
  let tilePath = "";
  if (lType === 'koppen') tilePath = `${tilePrefix}/tiles_koppen/{z}/{x}/{y}.png?v=3`;
  else if (lType === 'precip') tilePath = `${tilePrefix}/tiles_precip/${m}/{z}/{x}/{y}.png?v=3`;
  else if (lType === 'temp') tilePath = `${tilePrefix}/tiles_temp/${m}/{z}/{x}/{y}.png?v=3`;
  else if (lType === 'precip_max') tilePath = `${era}/tiles_precip_max/{z}/{x}/{y}.png?v=3`;
  else if (lType === 'precip_min') tilePath = `${era}/tiles_precip_min/{z}/{x}/{y}.png?v=3`;
  ...
  if (source && tilePath) {
    ...
    requestAnimationFrame(() => {
      if (map.getSource('data-source')) {
        ...
        if (map.getLayer('coordinate-line-border-layer')) map.moveLayer('coordinate-line-border-layer');
        if (map.getLayer('coordinate-line-layer')) map.moveLayer('coordinate-line-layer');
        map.triggerRepaint();
      }
    });
  } else if (source) {
...
function updateLegend(type, isCompare) {
  ...
  if (type === 'koppen') {
    ...
    const tempGradient = isCompare ?
      `linear-gradient(to right, hsl(240, 100%, 40%), hsl(120, 100%, 40%), hsl(0, 100%, 40%))` :
    ...
    if (isCompare) {
      ...
      contentEl.innerHTML = `
        <div class="gradient-bar" style="background: linear-gradient(to right, hsl(0, 100%, 40%) 0%, hsl(60, 100%, 40%) 11%, hsl(120, 100%, 40%) 31%, hsl(180, 100%, 40%) 56%, hsl(240, 100%, 40%) 83%, hsl(270, 100%, 40%) 100%);"></div>
    ...
    contentEl.innerHTML = `
      <div class="gradient-bar" style="background: linear-gradient(to right, rgb(194, 9, 86) 0%, rgb(204, 0, 199) 9%, rgb(122, 0, 204) 18%, rgb(0, 91, 204) 27%, rgb(6, 151, 197) 36%, rgb(2, 201, 148) 45%, rgb(2, 201, 33) 54%, rgb(146, 204, 0) 63%, rgb(204, 162, 0) 72%, rgb(204, 118, 0) 81%, rgb(204, 61, 0) 90%, rgb(204, 0, 0) 100%);"></div>
...
function triggerLocationQuery(lat, lng, isClick = false) {
  ...
  isPopupOpen = true;
  syncUrl();
  queryLocation(lat, lng, false, isClick);
}
...
function getTrewartha(t, p, lat) {
  ...
  if (annP < R_mm) {
    ...
    return { code: type + thermalSub, coldDesc, warmDesc, coldMonth, warmMonth, minT, maxT };
  }
  ...
  else return { code: (maxT > 0 ? "Ft" : "Fi") + thermalSub, coldDesc, warmDesc, coldMonth, warmMonth, minT, maxT };
...
function updatePopup() {
  ...
  if (isCompare) {
    ...
    const isLight = document.body.classList.contains('light-mode');
    const wetColor = isLight ? '#1f4e9c' : '#8da0cf';
    const dryColor = isLight ? '#b5382b' : '#f1948a';
    html += `
      <table style="width:100%; font-size:11px; border-collapse:collapse; text-align:center;">
        <tr style="color:var(--text-dim); border-bottom:1px solid var(--border);">
          ...
          ${months.map((m, i) => {
      let style = "", click = "", cursor = "cursor:default";
      if (i === maxIdx) { style = `color:${wetColor}; font-weight:bold; text-decoration:underline;`; click = `setLayer('precip_max')`; cursor = "cursor:pointer"; }
      else if (i === minIdx) { style = `color:${dryColor}; font-weight:bold; text-decoration:underline;`; click = `setLayer('precip_min')`; cursor = "cursor:pointer"; }
      return `<td style="padding: 0 2px; ${cursor}; ${style}" onclick="${click}">${m}</td>`;
        ...
        <tr style="cursor: pointer;" onclick="setLayer('temp', 12)">
          ...
          ${d1.t.map((t, i) => {
      const decoration = "text-decoration: underline; cursor: pointer;";
      const click = `event.stopPropagation(); setLayer('temp', ${i})`;
      return `<td style="color:${getTempColor(t)}; font-weight:bold; ${decoration}" onclick="${click}">${isMetric ? Math.round(t) : Math.round(t * 1.8 + 32)}</td>`;
        ...
        <tr style="cursor: pointer;" onclick="setLayer('precip', 12)">
          ...
          ${d1.p.map((p, i) => {
      const decoration = "text-decoration: underline; cursor: pointer;";
      const click = `event.stopPropagation(); setLayer('precip', ${i})`;
...
async function queryLocation(lat, lng, isRefresh = false, isClick = false) {
  ...
  const pixelX = px % 256, pixelY = py % 256;
  const pixel_x_norm = (px / map_size) - 0.5;
  const pixel_lon = 360 * pixel_x_norm;
  const pixel_y_norm = 0.5 - (py / map_size);
  const pixel_lat = 90 - 360 * Math.atan(Math.exp(-pixel_y_norm * 2 * Math.PI)) / Math.PI;

  const latC = Math.floor(pixel_lat / 10) * 10, lonC = Math.floor(pixel_lon / 10) * 10;
  const gridY = Math.min(1799, Math.max(0, Math.floor((90 - pixel_lat) / 180 * 1800)));
  const gridX = (Math.floor((pixel_lon + 180) / 360 * 3600) % 3600 + 3600) % 3600;
  ...
  try {
    ...
    // COASTLINE SNAP: If initial hit is water/transparent (kid1 is 0) or if we lack grid data for a solid land pixel, search.
    // However, on direct map clicks, if it is a truly transparent pixel (kid1 is 0), we bypass the snap to prevent showing popups on water.
    const isTransparent = !kid1;
    if (!(isClick && isTransparent) && (!kid1 || !d1_raw)) {
      let found = false;
      const radius = 3;
...
map.on('click', (e) => {
  ...
  triggerLocationQuery(e.lngLat.lat, e.lngLat.lng, true);
});
...
map.on('render', () => {
  ...
  const is3D = document.getElementById('terrain-select').value === '3d';
  if (is3D) {
    navSmoothX = navSmoothX * 0.8 + dx * 0.2;
    navSmoothY = navSmoothY * 0.8 + dy * 0.2;
  } else {
    navSmoothX = dx;
    navSmoothY = dy;
  }
  if (navSmoothX !== 0 || navSmoothY !== 0) {
    map.jumpTo({ zoom: Math.min(11, Math.max(1.5, map.getZoom() + (-navSmoothY * 0.0005))), bearing: map.getBearing() + (navSmoothX * 0.012) });
    if (middleStartLlngLat) {
      const rect = map.getCanvas().getBoundingClientRect();
      const rawProj = map.project(middleStartLlngLat);
      if (!smoothedProj || !is3D) smoothedProj = { x: rawProj.x, y: rawProj.y };
      else {
      ...
      const panFactor = is3D ? 0.8 : 1.0;
      const offX = (smoothedProj.x - (middleStartPos.x - rect.left)) * panFactor;
      const offY = (smoothedProj.y - (middleStartPos.y - rect.top)) * panFactor;
...
function showCoordinateLine(type, val) {
  activeLineType = type;
  activeLineVal = val;
  ...
  // Hide border layer if it exists
  if (map.getLayer('coordinate-line-border-layer')) {
    map.setLayoutProperty('coordinate-line-border-layer', 'visibility', 'none');
  }

  if (!map.getLayer('coordinate-line-layer')) {
    map.addLayer({
      id: 'coordinate-line-layer',
      type: 'line',
      source: 'coordinate-line-source',
      paint: {
        'line-color': '#888888',
        'line-width': 2,
        'line-opacity': 0.8
    ...
    map.setLayoutProperty('coordinate-line-layer', 'visibility', 'visible');
    map.setPaintProperty('coordinate-line-layer', 'line-color', '#888888');
    map.setPaintProperty('coordinate-line-layer', 'line-width', 2);
  }

  // Move coordinate-line-layer to the top of all layers
  if (map.getLayer('coordinate-line-layer')) {
    map.moveLayer('coordinate-line-layer');
  }
}

function hideCoordinateLine() {
  activeLineType = null;
  activeLineVal = null;
  const source = map.getSource('coordinate-line-source');
  if (source) {
  ...
  if (map.getLayer('coordinate-line-layer')) {
    map.setLayoutProperty('coordinate-line-layer', 'visibility', 'none');
  }
}

function toggleCoordinateLine(type, val) {
  if (activeLineType === type && activeLineVal === val) {
    hideCoordinateLine();
  } else {
    showCoordinateLine(type, val);
  }
}

window.toggleCoordinateLine = toggleCoordinateLine;
window.hideCoordinateLine = hideCoordinateLine;
